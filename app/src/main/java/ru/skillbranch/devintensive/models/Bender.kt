...
class Bender(var status:Status = Status.NORMAL, var question:Question = Question.NAME) {
    ...
    fun listenAnswer(answer:String) : Pair<String, Triple<Int, Int, Int>>{
        ...
        return if (question.validateAnswer(answer)) {
            if (question.answers.contains(answer.toLowerCase()) || question.answers.isEmpty()) {
    ...
    enum class Question(val question:String, val answers:List<String>, val validate:String){
        ...
        SERIAL("Мой серийный номер?", listOf("2716057"), "Серийный номер содержит только цифры, и их 7"){
            ...
            override fun validateAnswer(answer:String): Boolean {
                var chars = answer.toCharArray()
                val (digits, notDigits) = chars.partition { it.isDigit() }
                return (notDigits.isEmpty() && (digits.size == 7))
            }
        },
        IDLE("На этом все, вопросов больше нет", listOf(), ""){
            override fun nextQuestion(): Question = IDLE
            override fun validateAnswer(answer: String) = true
        };
